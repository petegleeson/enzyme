 ...
 * @param {Node} root
 */
function matchAdjacentSiblings(nodes, predicate) {
  return nodes.reduce((matches, node) => {
    ...
    const adjacentSibling = parent.children[nodeIndex + 1];
    // No sibling
    if (!adjacentSibling) {
      return matches;
    }
 ...
 * @param {Node} root
 */
function matchGeneralSibling(nodes, predicate) {
  return uniqueReduce((matches, node) => {
    const parent = node.parent;
    const nodeIndex = parent.children.indexOf(node);
    parent.children.forEach((sibling, i) => {
...
function matchDescendant(nodes, predicate) {
  return uniqueReduce((matches, node) => matches.concat(node.findAll(predicate)), nodes);
}
...
export function reduceTreeBySelector(selector, root) {
  ...
  if (typeof selector === 'function' || typeof selector === 'object') {
    results = root.findAll(buildPredicate(selector));
    ...
    while (index < tokens.length) {
      ...
      if (token.type === SELECTOR) {
        ...
        results = results.concat(root.findAll(predicate));
      } else {
        ...
        switch (type) {
          ...
          case ADJACENT_SIBLING:
            results = matchAdjacentSiblings(results, predicate);
            break;
          // The ~ combinator
          case GENERAL_SIBLING:
            results = matchGeneralSibling(results, predicate);
            break;
          // The > combinator
          case CHILD:
